...
const resolveFile = (appDirectory: string, configPath: string, fileName: string): string => {
  ...
  if (fs.existsSync(path.resolve(appDirectory, configPath, `${fileName}.ts`))) {
    debug('resolved config file', fileName, path.resolve(appDirectory, configPath, `${fileName}.ts`));
    ...
    debug('resolved config file', fileName, path.resolve(appDirectory, configPath, `${fileName}.js`));
...
const loadSecrets = (
  ...
  if (secretNames)
    convertToArray(secretNames).forEach((secretName) => mergedSecretNames.add(secretName));
  ...
  secrets.forEach((secret) => Object.assign(mergedSecrets, secret));
